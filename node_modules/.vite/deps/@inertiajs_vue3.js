...
function copyProperties(target, source, objectToClone = target, stack, cloneValue) {
  ...
  for (let i = 0; i < keys2.length; i++) {
    ...
    if (descriptor == null || descriptor.writable) {
      target[key2] = cloneDeepWithImpl(source[key2], key2, objectToClone, stack, cloneValue);
    }
  }
}
...
function findKey(obj, predicate) {
  ...
  return keys2.find((key2) => predicate(obj[key2], key2, obj));
}
    ...
    switch (aTag) {
      case mapTag: {
        ...
        for (const [key2, value] of a.entries()) {
          if (!b.has(key2) || !isEqualWithImpl(value, b.get(key2), key2, a, b, stack, areValuesEqual)) {
            return false;
          }
        }
        return true;
...
function isObjectMatch(target, source, compare, stack) {
  ...
  try {
    for (let i = 0; i < keys2.length; i++) {
      ...
      if (source[key2] === void 0 && target[key2] !== void 0) {
        return false;
      }
      if (source[key2] === null && target[key2] !== null) {
        return false;
      }
      const isEqual2 = compare(target[key2], source[key2], key2, target, source, stack);
      if (!isEqual2) {
        return false;
      }
    }
...
function isMapMatch(target, source, compare, stack) {
  ...
  for (const [key2, sourceValue] of source.entries()) {
    const targetValue = target.get(key2);
    const isEqual2 = compare(targetValue, sourceValue, key2, target, source, stack);
    if (isEqual2 === false) {
      return false;
    }
  }
...
function every(source, doesMatch, guard) {
  ...
  if (!isArrayLike(source)) {
    ...
    for (let i = 0; i < keys2.length; i++) {
      ...
      if (!predicate(value, key2, source)) {
        return false;
      }
    }
    return true;
...
function find(source, _doesMatch = identity, fromIndex = 0) {
  ...
  if (typeof doesMatch === "function" && !Array.isArray(source)) {
    ...
    for (let i = fromIndex; i < keys2.length; i++) {
      const key2 = keys2[i];
      const value = source[key2];
      if (doesMatch(value, key2, source)) {
        return value;
      }
    }
    return void 0;
...
function assignImpl(object, source) {
  ...
  for (let i = 0; i < keys$1.length; i++) {
    const key2 = keys$1[i];
    if (!(key2 in object) || !eq(object[key2], source[key2])) {
...
function prototypeKeysIn(object) {
  ...
  return keys2.filter((key2) => key2 !== "constructor");
}
...
function assignInWithImpl(object, source, getValueToAssign) {
  ...
  for (let i = 0; i < keys2.length; i++) {
    ...
    const newValue = (getValueToAssign == null ? void 0 : getValueToAssign(objValue, srcValue, key2, object, source)) ?? srcValue;
...
function cloneStringObjectProperties(target, source) {
  ...
  for (const key2 in source) {
    if (Object.hasOwn(source, key2) && (Number.isNaN(Number(key2)) || Number(key2) >= stringLength)) {
      target[key2] = source[key2];
    }
  }
}
...
function findLastKey(obj, predicate) {
  ...
  const iteratee$1 = iteratee(predicate ?? identity2);
  const keys2 = Object.keys(obj);
  return keys2.findLast((key2) => iteratee$1(obj[key2], key2, obj));
}
