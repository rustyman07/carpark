...
class ParkoutReportGenerator
{
    public static function generate($tickets, $totalParkFee, $totalTickets, $reportDate, $company)
    {
        $rows = '';
        ...
        foreach ($tickets as $index => $ticket) {
            ...
            $parkIn = $ticket->park_datetime
                ? Carbon::parse($ticket->park_datetime)->format('M d, Y h:i:s A')
                : 'N/A';

            $parkOut = $ticket->park_out_datetime
                ? Carbon::parse($ticket->park_out_datetime)->format('M d, Y h:i:s A')
                : 'N/A';
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .signature-grid { display: flex; gap: 32px; margin-top: 32px; justify-content: flex-start; }
                .signature-item { text-align: left; }
                .signature-line { border-top: 2px solid #1f2937; padding-top: 8px; margin-top: 48px; width: 200px; }
                .signature-label { font-size: 14px; font-weight: 600; }
                .signature-role { font-size: 11px; color: #6b7280; margin-top: 4px; }
                .footer-note { text-align: left; font-size: 11px; color: #6b7280; margin-top: 32px; padding-top: 16px; border-top: 1px solid #e5e7eb; width: fit-content; }
            </style>
        </head>
        <body>
            <div class='header'>
