				...
				>
					...
					<template v-slot:item.payment_method="{ item }">
						<div class="d-flex align-center justify-center payment-method-cell">
							<span
								...
								:style="{
									backgroundColor: getPaymentMethodDotColor(
										item.payment_method,
									),
								}"
							></span>
					...
					<template v-slot:body.append>
						<tr class="summary-row">
							...
							<td class="text-right font-weight-bold text-indigo-darken-4">
					...
					<v-card-title class="bg-indigo-darken-4 text-white">
						<div class="d-flex align-center justify-space-between">
							<div class="d-flex align-center">
								...
								<span class="text-h6"
									>Payment Details - #{{ selectedPayment?.id }}</span
								>
							</div>
							<v-btn
								icon
					...
					<v-card-text class="pa-6" v-if="selectedPayment">
							...
							<v-row>
								...
								<v-col cols="12" sm="6">
									<div class="detail-item">
										...
										<span class="detail-value"
											>{{ selectedPayment.gcash_reference || "" }}
										</span>
									</div>
								</v-col>
								<v-col cols="12" sm="6">
									<div class="detail-item">
										...
										<span
											class="detail-value font-weight-bold text-indigo-darken-4"
										>
								...
								<v-col cols="12" sm="6">
									<div class="detail-item">
										...
										<v-chip
											:color="
												selectedPayment.status === 'Paid' ? 'success' : 'error'
											"
											variant="flat"
											size="small"
											class="ml-2"
						...
						<div v-else>
							<v-alert type="info" variant="tonal" class="mt-4">
								No additional card payment details available for this
								transaction.
							</v-alert>
						</div>
					</v-card-text>
...
const getTodayCount = () => {
	...
	return (
		props.payments.filter(
			(p) => dayjs(p.paid_at).format("YYYY-MM-DD") === today,
		).length || 0
	);
};
