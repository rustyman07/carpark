	...
	<div class="receipt-wrapper">
		...
		>
			<v-card class="dialog-card">
				<v-card-title
					class="d-flex justify-space-between align-center pa-4 bg-indigo-darken-4"
				>
				...
				<v-card-text class="pa-0 pdf-container">
					...
					<div
						v-else
						class="d-flex align-center justify-center loading-container"
					>
						<v-progress-circular
							indeterminate
			...
			<v-row justify="center">
				<v-col cols="12" md="8" lg="6">
					...
					<div class="success-header mb-6 text-center">
						...
						<h1
							color="success"
							class="text-h4 font-weight-bold text-white mb-2 text-success"
						>
							Payment Successful!
						</h1>
					</div>
					...
					<v-row class="receipt-cards">
						...
						<v-col cols="12" md="6">
							<v-card
								ref="receiptContent"
								class="receipt-card elevation-2"
								rounded="sm"
							>
								<v-divider></v-divider>
										...
										>
											<div class="pa-4">
												...
												<div class="info-row mb-3 flex w-full justify-between">
													...
													<span class="value">
														{{
															props.ticket.vehicle_type
																.charAt(0)
																.toUpperCase() +
															props.ticket.vehicle_type.slice(1)
														}}
													</span>
												</div>
							...
							<v-card class="receipt-card elevation-2" rounded="sm">
								...
								<div class="receipt-body pa-6 text-sm text-indigo-darken-4">
									...
									<div class="section mb-6">
										...
										<v-card
											class="payment-card elevation-0"
											color="grey-lighten-5"
											rounded="lg"
										>
											<div class="pa-4">
												<div class="info-row mb-3 flex w-full justify-between">
													...
													<span class="value">{{
														formatCurrency(props.ticket.park_fee)
													}}</span>
												</div>

												<div
													v-if="payment.has_discount"
													class="info-row mb-3 flex w-full justify-between"
												>
													<span class="label">Discounted</span>
													<span class="value">(Senior, PWD)</span>
												</div>
												<!-- Cards Used -->
												<div
													v-if="filteredCard.length"
													class="cards-section my-4"
												>
												...
												<!-- Cash Payment -->
												<div
													v-if="filteredCash.length"
													class="cash-section my-4"
												>
													<v-divider class="mb-3"></v-divider>
													<div
														v-for="item in filteredCash"
														:key="item.id"
														class="info-row mb-2"
													>
														<div class="d-flex align-center">
...
const duration = computed(() => {
	if (!props.ticket?.park_datetime || !props.ticket?.park_out_datetime)
		return null;
