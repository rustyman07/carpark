			...
			<v-card class="data-table-card elevation-8" rounded="lg">
				...
				>
					...
					<template v-slot:item.park_out_datetime="{ item }">
						<div
							v-if="item.park_out_datetime && item.park_out_datetime !== '-'"
		...
		>
			<v-card class="dialog-card">
				...
				>
					...
					<v-btn icon variant="text" @click="closeTicketDialog" color="white">
		...
		>
			<v-card class="dialog-card">
				...
				>
					...
					<v-btn icon variant="text" @click="closePrintDialog" color="white">
						<v-icon>mdi-close</v-icon>
					</v-btn>
				</v-card-title>
			...
			<v-card rounded="lg">
				...
				<v-card-text class="pa-6">
					<v-alert type="warning" variant="tonal" density="comfortable">
						Are you sure you want to void this ticket? This action cannot be
						undone.
					</v-alert>
				</v-card-text>
				<v-divider></v-divider>
...
const headers = [
	...
	{ key: "vehicle_type", title: "Vehicle Type", align: "center" },
...
const printTicket = (item) => {
	...
	if (item.uuid) {
		...
		console.log(
			...
			route("print.ticket", { uuid: item.uuid }),
		);
...
const printReceipt = (item) => {
	...
	if (item.uuid) {
		...
		console.log(
			...
			route("receipt.print", { uuid: item.uuid }),
		);
